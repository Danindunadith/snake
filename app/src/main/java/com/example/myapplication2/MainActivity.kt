...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val daniBtn = findViewById<Button>(R.id.ins)
        daniBtn.setOnClickListener {
        ...
        // Define the levels and their corresponding delay values (speeds)
        val levels = listOf(30L, 20L, 10L)
        var currentLevel = 0 // Start at level 0

        // Function to update the snake's speed based on the current level
        fun updateSnakeSpeed() {
            delayMillis = levels[currentLevel]
        }

// Button to increase the level (increase speed)
        findViewById<Button>(R.id.levelUp).setOnClickListener {
...
// Button to decrease the level (decrease speed)
        findViewById<Button>(R.id.levelDown).setOnClickListener {
            if (currentLevel > 0) {
                currentLevel--
                updateSnakeSpeed()
                }
        ...
        newgame.setOnClickListener {
            ...
            fun checkFoodCollision() {
                ...
                if (distance < distanceThreshold) { // Check if the distance between the snake head and the meat is less than the threshold
                    ...
                    //i added related high score

                    scorex++
                    score2.text = "Score: $scorex" // Update the displayed score
...
// Set button onClickListeners to update the currentDirection variable when pressed
    ...
    private fun updateHighScore(score: Int) {
        if (score > highScore) {
            ...
            // Update the UI to display the new high score
            findViewById<Button>(R.id.score2).text = "High Score: $highScore"
        }
    }
